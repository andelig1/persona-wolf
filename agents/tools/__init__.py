...
def create_tools_for_role(role: str, memory, game_state_provider,
    ...
    """根据角色创建完整工具集（通用工具 + Agent主动工具 + 角色专属工具）"""
    common = create_common_tools(memory, game_state_provider, inference_engine)
    agent = create_agent_tools(memory, game_state_provider, inference_engine)
    role_tool_map = {
    ...
    return common + agent + factory(memory, game_state_provider, inference_engine)
